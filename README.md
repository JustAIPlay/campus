...
### 💬 智能对话系统
- **目标训练模式**：结构化的社交技能训练，包含明确的学习目标和阶段性指导
  - 🎯 **训练目标**：每个场景都有具体的社交技能目标（如还原事实、表达感受、寻找双赢方案）
  - 📊 **阶段进度**：分为开场、探索、协商、结束、反思五个阶段，循序渐进
  - 💡 **内联建议**：实时提供智能建议气泡，一键填充合适的回复内容
  - 🤖 **AI智能对话**：基于大语言模型的自然对话，提供个性化反馈
- **自由聊天模式**：开放式的对话练习，自由选择场景和角色进行交流
  - 🗣️ **自由对话**：不受限制的开放式交流，培养自然对话能力
  - 🎭 **角色扮演**：与不同性格的NPC进行多样化互动
  - 📝 **预设话术**：提供4-5个常用对话选项作为参考
- **情境匹配**：对话内容与场景高度匹配
- **社交技能训练**：涵盖问候、求助、道歉、感谢、冲突解决等多种社交场景
...
### 🎓 目标训练模式
1. **开始训练**：在首页点击"开始目标训练"按钮
2. **了解目标**：查看当前训练场景的具体目标和要求
3. **阶段学习**：按照开场→探索→协商→结束→反思的五个阶段进行
4. **获取建议**：点击内联建议气泡获取智能回复建议
5. **AI对话**：与AI进行自然对话，获得实时反馈和指导
6. **完成反思**：在反思阶段总结学习成果

### 💬 自由聊天模式
1. **进入模式**：在首页点击"自由聊天模式"按钮
2. **选择场景**：选择想要体验的校园场景
3. **选择角色**：在场景中选择想要对话的NPC角色
4. **开始对话**：使用预设话术或自由输入进行互动
5. **自由练习**：不受限制地练习各种社交场景

## 🆕 最新更新

### v0.2.1 (2025-01)
- ✅ **冲突协商策略增强**：优化AI对话中的冲突处理机制
  - 🎯 **分歧处理规则**：当出现胜负不确定或分歧时，自动触发"数到三同时出"和"三局两胜"机制
  - 🛡️ **防循环护栏**：连续两次未确认胜负或无共识时，自动切换为"2分钟计时轮换"并明确开始者
  - 🤝 **协商优化**：提升冲突场景下的对话质量和解决效率
- ✅ **UI显示问题修复**：解决首页"开始训练"按钮仅悬停可见的问题
- ✅ **系统稳定性提升**：优化错误处理和异常情况的用户体验

### v0.2.0 (2025-01)
- ✅ **目标训练模式**：实现结构化的社交技能训练系统
  - 🎯 训练目标展示：清晰展示每个场景的学习目标
  - 📊 阶段进度跟踪：五阶段训练流程（开场→探索→协商→结束→反思）
  - 💡 内联建议系统：智能建议气泡，支持一键填充回复
  - 🤖 AI智能对话：集成大语言模型，提供个性化反馈
- ✅ **自由聊天模式**：开放式对话练习功能
- ✅ **双模式切换**：首页提供"开始目标训练"和"自由聊天模式"两个入口
- ✅ **运行时错误修复**：解决currentScenario空值检查问题，提升应用稳定性
...
**已完成任务 (2/15)**
- ✅ 快捷回复与内联建议：在输入框附近展示智能建议气泡，支持一键填充回复
- ✅ 场景状态管理重构：完成 scenario/phase 状态接入，保持新旧系统兼容性

**进行中任务 (1/15)**
- 🔄 训练目标与阶段进度展示：正在开发训练目标界面和内联建议交互组件

**待完成任务 (12/15)**

#### 🔴 最高优先级任务 (5项)
- ⏳ **后端代理化**：通过后端代理 /api/chat 转发硅基流动请求，隐藏前端 API Key
- ⏳ **资源路径统一**：全量改用 /images-webp/... 且采用绝对路径
- ⏳ **CSS 体积优化**：将 index.css 调整为 @tailwind 指令 + 内容扫描（JIT）
- ⏳ **依赖版本锁定**：将 clsx、tailwind-merge 等使用"*"的依赖固定具体版本
- ⏳ **结构拆分**：将体量较大的 App.tsx 拆分为场景配置 + UI 组件 + 对话容器

#### 🟡 中等优先级任务 (5项)
- ⏳ **流式输出与可取消**：启用流式渲染（逐字显示）与"停止生成/继续生成"控制
- ⏳ **错误与离线可练习模式**：AI 未配置或请求失败时，自动切换到"离线练习"规则回复
- ⏳ **首次使用引导与场景目标提示**：提供 2–3 步新手引导
- ⏳ **移动端输入与可访问性**：支持 Enter 发送/Shift+Enter 换行、输入自适应高度
- ⏳ **视觉与可读性优化**：背景图叠加轻度遮罩提升对比度，消息气泡细节优化

#### 🟢 长期建设任务 (4项)
- ⏳ **学习目标与回顾闭环**：会话开始选择目标，对话中给微反馈标签，会话结束提供总结
- ⏳ **会话存档与复训**：保存练习记录、支持回放与分享、支持"继续上次目标"的复训入口
- ⏳ **指标与 A/B 测试**：关键指标包含 TTFM（首条消息用时）、建议点击率、对话轮次、完成率
- ⏳ **内容安全与年龄适配**：在系统提示中明确校园语境与不当内容过滤
...
#### 体验提升（中优先级）
- **快捷回复与内联建议**：✅ 已完成 - 在输入框附近展示 3–5 个与 NPC/场景相关的话术快捷气泡；将"获取建议"从弹窗改为上下文内联，点击即插入或一键发送，减少上下文切换。
